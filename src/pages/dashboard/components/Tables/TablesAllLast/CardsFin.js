...
function CardsLast() {
  ...
  var columns = [
    ...
    {title: "Data", field: "i3_date_auto",type: "date", render: rowData => moment(rowData.i3_date_auto).format('DD/MM/YYYY')},
  ...
  useEffect(() => { 
    axiosInstance.get('carddataall/' + columns[16]['field'])
        .then(res => {               
            setData(res.data)
  ...
  return (
    <div className="App">
      <Grid container spacing={1}>
          ...
          <Grid item xs={12}>
            <MaterialTable
              //title= {'Últimos 5 ' + columns[18]["title"] + 's'}
              title= {'Últimos 5 Finalizados'}
