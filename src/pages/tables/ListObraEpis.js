...
function Boards() {
  ...
  var columns = [
    ...
    {title: "Obra", field: "card", filtering: true, render:rowData=>  <Link href={`/app/epis/${rowData.card}`}>{rowData.card}</Link>},
  ...
  let decoded = decodeURIComponent('teste1%201');
  console.log(decoded)
  
  useEffect(() => { 
    axiosInstance.get('boardepis/')
        .then(res => {               
            setData(res.data)
